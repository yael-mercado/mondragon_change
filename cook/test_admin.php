...
//  Display the course home page.
    ...
    $where_attemp = " WHERE t.test_id = (select t_id from test_quiz where id_course = {$course->id})";

    $sql_user_attemps ="SELECT * FROM (SELECT max(mdl_munduaattemps.a_id) as a_id, mdl_munduaattemps.user_id, lastname, firstname, max(fecha) as fecha, count(*) as intentos,
                        (select  puntaje from mdl_munduaattemps as at where at.a_id = max(mdl_munduaattemps.a_id) )as puntaje, email, test_id from mdl_munduaattemps
                        inner join mdl_munduauser on mdl_munduaattemps.user_id = mdl_munduauser.id
                        group by test_id, user_id, lastname, firstname, email
                        order by mdl_munduaattemps.a_id desc) AS t".$where_attemp;


    $intentos = $DB->get_recordset_sql($sql_user_attemps);
    $rect="";
    //$links = "";
    //die(var_dump($sql_user_attemps));
    $table_user_attemps.="<tbody>";

    $gropsAll = groups_get_all_groups($course->id);
    foreach ($gropsAll as $key => $value) {
      $options .= "<option value='".$value->id."' ".($value->id == $_GET['group'] ? "selected" : "").">".$value->name."</option>";
    }
    //die(var_dump($options));
      $user_visible = 0;
      foreach ($intentos as $key => $intento) {
        $grupos = groups_get_user_groups($course->id, $intento->user_id);
        ...
        $table_user_attemps.="<tr>";
          $table_user_attemps.="<td>".$intento->user_id."</td>";
          $table_user_attemps.="<td>".$intento->firstname." ".$intento->lastname."</td>";
          $table_user_attemps.="<td>".$intento->email."</td>";
          $table_user_attemps.="<td>".$rol."</td>";
          $table_user_attemps.="<td>".$grupo."</td>";
          $table_user_attemps.="<td>".$intento->fecha."</td>";
          $table_user_attemps.="<td>".$intento->puntaje."</td>";
          $table_user_attemps.="<td>".$intento->intentos."</td>";
            $links .= '<a href="/cook/result_admin.php?id='.$course->id.'&user_id='.$intento->user_id.'" style="color: black; margin:2px;" title="Ver resultado"><i class="fas fa-poll fa-2x"></i></a>';
            $links .= '<a href="/cook/Export_excel.php?id='.$course->id.'&user_id='.$intento->user_id.'" style="color: black; margin:2px;" title="Exportar XlS"><i class="far fa-file-excel fa-2x"></i></i></a>';
      ...
      if (!$user_visible){
        $table_user_attemps.="<tr>";
          $table_user_attemps.="<td colspan='9' style='text-align: center;'>No hay registros</td>";
        $table_user_attemps.="</tr>";
      }
      //exit;
  ...
  <BODY>
    <div class="" style="box-shadow: 0 0 20px rgb(0 0 0 / 10%);
      ...
      <div class="row" style="min-height: 70%; height: 100%;">
              ...
              <div class="row">
                <div class="col-md-12 text-center" style="margin:auto;">
                  <div class="row">
                    ...
                    <div class="col-md-10">
                      <div class="col-md-12">
                        <h6>Usuarios con intentos</h6>
                      </div>
                      <div class="row">
                        <div class="col-md-3">
                          <label for="">Mostrar:</label>
                          <select class="filter_m" name="filter_m" id="filter_m">
                        ...
                        <div class="col-md-3">
                          <a class="btn btn-default" title="Exportar todos los usuarios" href="Export_excelAll.php?id=<?= $course->id?>&group=<?= $_GET['group'] ?>" style="color:black;"><i class="far fa-file-excel fa-2x"></i></a>
                          <a class="btn btn-default" title="Configuración de preguntas" href="/cook/config_questions.php?id=<?= $course->id?>" style="color:black;"><i class="fas fa-cogs fa-2x"></i></a>
                        </div>
                      </div>
                  ...
                  <div class="row">
                    ...
                    <div class="table-resposive col-md-10">
                      <table class="table">
                        <thead>
                          <td>#</td>
                          <td>Nombre</td>
    ...
    <style>
      ...
      $(document).ready(function (){
        ...
        $('.filter_m').change(function (e){
          var id = $(this).val();
          window.location.href = "https://empleabilidad.redmundua.com/cook/test_admin.php?id=<?=$course->id?>&group="+id;
        });
      })
      </script>
  </BODY>
