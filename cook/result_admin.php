...
//  Display the course home page.
    ...
    inner join test_competencia on test_competencia.c_id = mdl_munduatest_questions.id_competencia where attempt_id = (select max(a_id) from mdl_munduaattemps where user_id = {$user_id_view} and test_id = (select t_id from test_quiz where id_course = {$course->id})) group by mdl_munduatest_questions.id_competencia";
    ...
    foreach ($ultimo_intento as $key => $value) {
      ...
      $rect .="<div class='col-md-12'>";
        $rect .= "<div class='row'>";
        $rect .="<div class='col-md-3' style='margin: auto;'>";
          $rect .="<h7><strong>".$value->name."</strong></h7>";
          $rect .="<p>".$value->definicion."</p>";
        $rect .= "</div>";
        $rect .="<div class='col-md-9' style='margin: auto;'>";
    ...
    if ($course->id != 5 && $course->id != 8 && $course->id != 9 ){
      ...
      inner join test_rel_options on (mdl_munduaanswer_users.question_id = test_rel_options.question_id and mdl_munduaanswer_users.option_id = test_rel_options.option_id)
      inner join test_competencia on test_competencia.c_id = mdl_munduatest_questions.id_competencia
      where attempt_id = (select max(a_id) from mdl_munduaattemps where user_id = {$user_id_view} and test_id in (select t_id from test_quiz where id_course = {$course->id})) group by attempt_id,(select fecha from mdl_munduaattemps
      where a_id = attempt_id),mdl_munduatest_questions.id_competencia,max_ponit,ponderado_point order by attempt_id,id_competencia";
      ...
      foreach ($competencias_rh as $key => $competencia) {
        ...
        FROM test_competencia as tet
        JOIN test_quiz on test_quiz.t_id = tet.test_id
        where id_course in (3) order by tet.c_id, porcentaje desc";
        //var_dump($sql_competencia);
        $competencias_rh = $DB->get_recordset_sql($sql_competencia);
        //$li_competencia .= "<div class='col-md-12 text-center'><h3>Competencias</h3></div>";
        foreach ($competencias_rh as $key => $competencia) {
          ...
          switch ($competencia->id_course) {
            ...
            case '3':
                $list_item .= "<li >".$competencia->name." ".$estrellaTexto."</li>";
            break;
            case '5':
              $li_competencia .= "<div class='col-md-12 text-center'>";
                $li_competencia .= "<h4 style='color: #a344c7;'>".$competencia->name."</h4>";
      ...
      inner join test_rel_options on (mdl_munduaanswer_users.question_id = test_rel_options.question_id and mdl_munduaanswer_users.option_id = test_rel_options.option_id)
      inner join test_competencia on test_competencia.c_id = mdl_munduatest_questions.id_competencia
      where attempt_id = (select max(a_id) from mdl_munduaattemps where user_id = {$user_id_view} and test_id in (1)) group by attempt_id,(select fecha from mdl_munduaattemps
      where a_id = attempt_id),mdl_munduatest_questions.id_competencia,max_ponit,ponderado_point order by attempt_id,id_competencia";
      $todos_intentos_araña = $DB->get_recordset_sql($sql_results_araña);
      ...
      inner join test_rel_options on (mdl_munduaanswer_users.question_id = test_rel_options.question_id and mdl_munduaanswer_users.option_id = test_rel_options.option_id)
      inner join test_competencia on test_competencia.c_id = mdl_munduatest_questions.id_competencia
      where attempt_id = (select max(a_id) from mdl_munduaattemps where user_id = {$user_id_view} and test_id in (3)) group by attempt_id,(select fecha from mdl_munduaattemps
      where a_id = attempt_id),mdl_munduatest_questions.id_competencia,max_ponit,ponderado_point order by attempt_id,id_competencia";
      ...
      and mdl_munduaanswer_users.option_id = test_rel_options.option_id)
      inner join test_competencia on test_competencia.c_id = mdl_munduatest_questions.id_competencia where attempt_id =
      (select max(a_id) from mdl_munduaattemps where user_id = {$user_id_view} and mdl_munduaattemps.test_id = (tet.test_id)) and test_competencia.c_id = tet.c_id) as porcentaje
      FROM test_competencia as tet
      JOIN test_quiz on test_quiz.t_id = tet.test_id
      ...
      inner join test_rel_options on (mdl_munduaanswer_users.question_id = test_rel_options.question_id and mdl_munduaanswer_users.option_id = test_rel_options.option_id)
      inner join test_competencia on test_competencia.c_id = mdl_munduatest_questions.id_competencia
      where attempt_id = (select max(a_id) from mdl_munduaattemps where user_id = {$user_id_view} and test_id in (2)) group by attempt_id,(select fecha from mdl_munduaattemps
      where a_id = attempt_id),mdl_munduatest_questions.id_competencia,max_ponit,ponderado_point order by attempt_id,id_competencia";
    ...
    <div class="" style="box-shadow: 0 0 20px rgb(0 0 0 / 10%);
      ...
      <nav class="row fixed-top navbar navbar-light navbar-expand moodle-has-zindex" style="background: white !important;">
        ...
        <div class="col-md-1">
          ...
          <a href="/cook/test_admin.php?id=<?= $course->id ?>" type="button" style="width: 100%; color: black; background: white !important; border-radius: 7px; border: solid 1px gray;" class="btn btn-default btn-video2">
